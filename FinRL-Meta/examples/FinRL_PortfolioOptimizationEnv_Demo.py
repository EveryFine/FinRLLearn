...
"""
-------------------------------------------------
   File Name：     FinRL_PortfolioOptimizationEnv_Demo
   Description :
   Author :       EveryFine
   Date：          2024/11/23
-------------------------------------------------
...
class PortfolioOptimizationEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self._terminal:
            ...
            print(
                "Final accumulative portfolio value: {}".format(self._portfolio_value / self._asset_memory['final'][0]))
            ...
            # load next state
            self._time_index += 1
            self.state, self.info = self._get_state_and_info_from_time_index(self._time_index)
    ...
    def _temporal_variation_df(self, periods=1):
        ...
        for column in self._features:
            ...
            df_temporal_variation[prev_column] = df_temporal_variation.groupby(self._tic_column)[column].shift(
                periods=periods)
